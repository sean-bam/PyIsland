...
parser.add_argument("-i", "--input", help="Path/to/input", required=False)
...
def get_gap_cols_from_msa(df, grcut=1):
    ...
    gap_col_list = gap_count_series.loc[
        lambda s: s >= max_gaps_in_column
    ].index.tolist()
...
def calc_homogeneity(aa, score, nseqs, expectaascores, blosum62):
    ...
    homogeneity = (score / nseqs - expectaascores[aa]) / (
        blosum62[aa, aa] - expectaascores[aa]
    )
...
def filter_msa(
    ...
    if hocut >= 0 or conplus:
        ...
        for col in range(0, ncols):
            ...
            homo = calc_homogeneity(
                bestaa, bestaascore, nseqs, expectedAAscores, blosum62
            )
