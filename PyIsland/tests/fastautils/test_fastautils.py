...
def test_subseq_fasta_biopython(get_test_fasta):
    ...
    expected_output = Path(
        "PyIsland/tests/fastautils/subseq_fasta_expected_output.fasta"
    )

    FastaUtils.subseq_fasta_biopython(seqs_to_get, get_test_fasta, outfasta)
...
def test_slice_fasta_biopython(get_test_fasta):
    ...
    expected_output = Path(
        "PyIsland/tests/fastautils/slice_fasta_expected_output.fasta"
    )

    FastaUtils.slice_fasta_biopython(seqs_to_get, start, stop, get_test_fasta, outfasta)
...
def test_fa_strict(get_test_fasta):
    ...
    FastaUtils.fa_strict(get_test_fasta, outfasta, min_len=5)
